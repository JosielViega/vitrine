...
WHERE p.active = 1 AND s.active = 1 AND c.active = 1
...
SQL;
    ...
    public function activeCategories(): array
    {
        return $this->database->connection()->query(
            'SELECT id, name, sort_order, active FROM categories WHERE active = 1 ORDER BY sort_order, name',
        )->fetchAll();
    }
    ...
    {
        return $this->database->connection()->query(
            'SELECT s.id, s.category_id, s.name, s.sort_order, s.active FROM subcategories s '
            . 'INNER JOIN categories c ON c.id = s.category_id '
            . 'WHERE s.active = 1 AND c.active = 1 ORDER BY c.sort_order, s.sort_order, s.name',
        )->fetchAll();
    }
